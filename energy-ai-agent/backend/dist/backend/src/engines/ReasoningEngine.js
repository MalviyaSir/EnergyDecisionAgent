...
export class ReasoningEngine {
    analyze(records, observationsResponse) {
        if (records.length < 2) {
...
function createReasoningItem(observation, context, index) {
...
function reasonForObservation(observation, context) {
...
function reasonPeakUsage(observation, context) {
    const evidence = [
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `Usage peaked because cooling-sensitive load increased while the building was operating in hot conditions. ` +
...
function reasonHighAppliance(observation, context) {
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `The appliance totals show one category carrying the largest share of demand. ` +
...
function reasonNightOrIdleLoad(observation, context) {
    const evidence = [
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `Consumption remains high even when occupancy is low, which points to always-on loads rather than occupant-driven usage. ` +
...
function reasonBattery(observation, context) {
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `The battery warning is explained by a demand and charging mismatch. ` +
...
function reasonSolarUnderuse(observation, context) {
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `Solar production exists in the dataset, but grid dependency remains high after the solar offset is applied. ` +
...
function reasonHighBill(observation, context) {
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `The billing risk comes from timing, not only total usage. ` +
...
function reasonCarbon(observation, context) {
    const evidence = [
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `Carbon emissions are driven by the amount of demand served by the grid after renewable generation is subtracted. ` +
...
function reasonSpike(observation, context) {
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `The day-over-day increase crossed the abnormal spike threshold, and the spike-day appliance mix shows a dominant load category. ` +
...
function reasonGeneric(observation, context) {
    const evidence = [
    ...
    return buildRuleResult({
        ...
        evidence,
        reasoning: `The observation was generated from smart meter behavior that crossed a deterministic threshold. ` +
...
function buildRuleResult({ observation, rootCause, primaryCause, secondaryCause, supportingFactors, evidence, reasoning, confidenceBoost, recommendedFocus, }) {
...
function buildMeterContext(records) {
    ...
    const totalApplianceUsage = records.reduce((total, record) => ({
    ...
    const lowOccupancyHighUsageRecord = (lowOccupancyRecords.length > 0 ? lowOccupancyRecords : records).reduce((peak, record) => record.energyConsumed > peak.energyConsumed ? record : peak);
    ...
    const highPriceThreshold = percentile(records.map((record) => record.electricityPrice), 0.75);
    ...
    const spike = records.slice(1).reduce((largest, record, index) => {
...
function buildSummary(reasoning) {
...
function calculateNightLoadPercent(records) {
