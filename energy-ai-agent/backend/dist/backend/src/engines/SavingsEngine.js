...
export class SavingsEngine {
    estimate(records, recommendationsResponse) {
        if (records.length < 2) {
        ...
        const weightedRoiScore = monthlySavings === 0
            ? 0
...
function estimateRecommendationSavings(recommendation, context) {
...
function buildContext(records, recommendationsResponse) {
...
function calculateRoiScore(monthlySavings, investmentCost, confidence, difficulty) {
...
function calculatePortfolioPayback(recommendations, monthlySavings) {
    const implementationCost = sum(recommendations, (recommendation) => implementationBaseCost[recommendation.difficulty] * riskMultiplier[recommendation.risk]);
...
function buildProjection(monthlySavings) {
    return Array.from({ length: 12 }).map((_, index) => ({
        month: `Month ${index + 1}`,
        cumulativeSavings: monthlySavings * (index + 1),
    }));
}
function buildDistribution(items, totalMonthlySavings) {
