...
export function simulateWhatIf(request) {
    return {
        status: 'placeholder',
        input: request,
...
export function recordFeedback(request) {
    return {
        status: 'accepted',
...
function calculateEfficiencyScore(record, solarRatio) {
...
function buildHourlyConsumption(record) {
...
function buildWeeklyUsage(records) {
...
function applianceEntries(usage) {
    return [
...
function buildApplianceHealth(record) {
    const appliances = [
    ...
    return appliances.map((appliance) => {
        ...
        const status = loadPercent > 88 ? 'Critical' : loadPercent > 68 ? 'High' : 'Normal';
        return {
            name: appliance.name,
...
function buildEnergySources(record) {
    ...
    return [
        { name: 'Grid', percentage: Math.round((grid / total) * 100) },
        { name: 'Solar', percentage: Math.round((solar / total) * 100) },
        { name: 'Battery', percentage: Math.round((battery / total) * 100) },
    ];
}
function buildLiveMeterFeed(records) {
