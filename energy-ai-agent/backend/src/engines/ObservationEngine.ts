import type { AgentObservation, ApplianceUsage, MeterDataRecord, ObservationsResponse } from '../../../shared/energy.js';

type Severity = AgentObservation['severity'];

type HourlyPoint = {
  record: MeterDataRecord;
  hour: number;
  consumption: number;
  occupancy: number;
};
...
export class ObservationEngine {
  analyze(records: MeterDataRecord[]): ObservationsResponse {
    if (records.length < 2) {
  ...
  private detectPeakUsageHours(hourly: HourlyPoint[]): AgentObservation {
  ...
  private detectHighConsumptionAppliance(records: MeterDataRecord[]): AgentObservation {
    const totals = records.reduce(
      (accumulator, record) => {
        Object.entries(record.applianceUsage).forEach(([appliance, value]) => {
          accumulator[appliance as keyof ApplianceUsage] += value;
        });
        return accumulator;
      },
      { hvac: 0, lighting: 0, refrigeration: 0, equipment: 0, evCharging: 0 },
    );
    const [key, value] = Object.entries(totals).sort((a, b) => b[1] - a[1])[0] as [keyof ApplianceUsage, number];
  ...
  private detectAbnormalSpikes(records: MeterDataRecord[]): AgentObservation[] {
    return records
      .slice(1)
      .map((record, index) => {
      ...
      .filter((observation): observation is AgentObservation => observation !== null);
  }

  private detectHighPriceUsage(records: MeterDataRecord[]): AgentObservation {
  ...
  private detectLowSolarUtilization(records: MeterDataRecord[]): AgentObservation {
    const solarDays = records.filter((record) => record.solarGeneration > 0);
    const latest = records.at(-1) as MeterDataRecord;
  ...
  private detectBatteryWarning(records: MeterDataRecord[]): AgentObservation {
  ...
  private detectNightConsumption(hourly: HourlyPoint[]): AgentObservation {
  ...
  private detectIdleConsumption(hourly: HourlyPoint[]): AgentObservation {
  ...
  private detectCarbonEmissionWarning(records: MeterDataRecord[]): AgentObservation {
  ...
  private detectPositiveEfficiency(records: MeterDataRecord[]): AgentObservation {
    const latest = records.at(-1) as MeterDataRecord;
    const previous = records.at(-2) as MeterDataRecord;
...
function createObservation(observation: AgentObservation): AgentObservation {
  return {
    ...observation,
...
function buildHourlyPoints(records: MeterDataRecord[]): HourlyPoint[] {
  const profileTotal = hourlyProfile.reduce((total, value) => total + value, 0);

  return records.flatMap((record) =>
    hourlyProfile.map((weight, hour) => ({
      record,
      hour,
...
function occupancyFactor(hour: number) {
  if (hour < 7 || hour > 21) {
    return 0;
  }
...
function severityFromPercent(value: number, criticalThreshold: number, highThreshold: number, mediumThreshold: number): Severity {
...
function applianceName(key: keyof ApplianceUsage) {
...
function dateId(timestamp: string) {
...
function calculateHealthScore(observations: AgentObservation[]) {
...
function statusFromScore(score: number): ObservationsResponse['overallStatus'] {
  if (score >= 90) {
    return 'Excellent';
  }
